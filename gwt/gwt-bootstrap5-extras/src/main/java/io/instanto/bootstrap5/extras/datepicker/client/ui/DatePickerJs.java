...
final class DatePickerJs {
  ...
  static TempusDominus create(
      final Element element,
      final String format,
      final boolean sideBySide,
      final boolean showClear,
      final boolean showClose) {
    final JsPropertyMap<Object> buttons =
        JsPropertyMap.<Object>of("today", true, "clear", showClear, "close", showClose);
    final JsPropertyMap<Object> display =
        JsPropertyMap.<Object>of("sideBySide", sideBySide, "buttons", buttons, "theme", "auto");
  ...
  static void bindChange(final TempusDominus picker, final ChangeHandler handler) {
    final String change =
        Js.asString(Js.global().nestedGet("tempusDominus.Namespace.events.change"));
    picker.subscribe(
        change,
        event -> {
  ...
  static double readValue(final TempusDominus picker) {
    ...
    return Js.isTruthy(dates) && dates.getLength() > 0
        ? Js.<DateTime>uncheckedCast(dates.getAt(0)).valueOf()
        : -1;
  }
