...
final class SortableJs {
  ...
  static void whenReady(Runnable action) {
    SortableResources.whenReady(SortableJs::isReady, action);
  }
  ...
  static Sortable create(Element el, String group, boolean enabled, PluginCallback moved) {
    ...
    options.setOnEnd(
        e ->
            moved.onEvent(
                e.getTo().getAttribute("data-sortable-owner")
                    + ":"
                    + e.getOldDraggableIndex()
                    + ":"
                    + e.getNewDraggableIndex()));
    return new Sortable(Js.asAny(el), opts);
  }

  static void enabled(Sortable p, boolean enabled) {
    p.option("disabled", !enabled);
  }

  static void destroy(Sortable p) {
    p.destroy();
  }

  @JsFunction
  interface EndListener {
    void handle(SortEvent event);
  }

  /**
   * The options that take callbacks. A typed property, not JsPropertyMap.set, because TeaVM makes a
   * lambda a JavaScript function only where its target type says so.
   */
  ...
  interface SortEvent {
    ...
    @JsProperty
    int getOldDraggableIndex();

    @JsProperty
    int getNewDraggableIndex();
  }
