...
final class GridJs {
    ...
    static final class Handle {
        ...
        private Handle(Tabulator table) { this.table = table; }
        private void whenBuilt(Runnable call) { if (built) call.run(); else pending.add(call); }
    }

    static void whenReady(Runnable action) { GridResources.whenReady(GridJs::isReady, action); }

    static boolean isReady() { return Js.global().get("Tabulator") != null; }

    static Handle create(Element el, String columns, String data, String group, boolean tree, int pageSize, String remote, PluginCallback changed) {
    ...
    static String data(Handle p) { return NativeJson.stringify(p.table.getData()); }
    ...
    static void destroy(Handle p) { p.table.destroy(); }
    ...
    @JsFunction interface TitleFormatter { Any format(); }
    @JsFunction interface GroupHeader { Any format(String value, int count); }
    ...
    interface Column {
        ...
        @JsProperty void setTitleFormatter(TitleFormatter formatter);
    }

    /** The table options that take callbacks. */
    @JsType(isNative = true)
    interface Options {
        @JsProperty void setGroupHeader(GroupHeader header);
    }
