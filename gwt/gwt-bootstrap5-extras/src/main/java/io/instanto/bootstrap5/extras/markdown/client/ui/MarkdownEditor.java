 ...
 *
 * <p>Deliberately not a rich text editor. A rich text editor keeps HTML, so an application storing
 * Markdown has to convert on every save, which is lossy and loses exactly the constructs a flexmark
 * server understands -- task lists and tables among them. This keeps the Markdown as the author
 * typed it, and adds what a bare textarea lacks: a toolbar that inserts the syntax, a preview
 * rendered with the same dialect the server uses, and the reference to hand rather than a link away
 * to another site.
 *
 * <p>The value is Markdown. {@link #getValue()} returns exactly what is in the textarea; {@link
 * #getHTML()} renders it.
 */
public class MarkdownEditor extends Div
    implements HasEnabled, HasId, HasValue<String>, HasValueChangeHandlers<String> {
  ...
  private Div syntaxButtons() {
    ...
    group.add(
        syntaxButton(IconType.TYPE_STRIKETHROUGH, "Strikethrough", "~~", "~~", "struck text"));
  ...
  /**
   * Shows the rendered Markdown, with a pencil to start editing. The syntax buttons and the
   * reference go with the textarea, because neither means anything while reading.
   */
  ...
  /**
   * Bootstrap 5's display utilities are declared !important, so the inline style that setVisible
   * writes cannot beat the d-flex on this container. The class has to come off for the buttons to
   * actually go away.
   */
