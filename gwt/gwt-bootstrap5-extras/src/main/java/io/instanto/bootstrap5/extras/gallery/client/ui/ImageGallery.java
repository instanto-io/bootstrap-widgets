...
public class ImageGallery extends PluginWidget<GalleryJs.Handle>
    implements HasSelectionHandlers<Integer> {
  public ImageGallery() {
    setStyleName("d-flex flex-wrap gap-3");
  }

  public void addImage(
      String source, String thumbnail, int width, int height, String alternativeText) {
    if (width <= 0 || height <= 0)
  ...
  public void close() {
    if (isReady()) GalleryJs.close(plugin());
  }
  ...
  protected void whenReady(Runnable action) {
    GalleryJs.whenReady(action);
  }

  @Override
  protected GalleryJs.Handle createPlugin() {
    return GalleryJs.create(
        getElement(), index -> SelectionEvent.fire(this, Integer.valueOf(index)));
  }

  @Override
  protected void destroyPlugin(GalleryJs.Handle plugin) {
    GalleryJs.destroy(plugin);
  }
}
