 ...
 */
public class SearchableSelect extends PluginWidget<SelectJs.Handle>
    implements HasValue<List<String>>, HasEnabled {
  ...
  public void setMultiple(boolean multiple) {
    ...
    select.setMultipleSelect(multiple);
  }
  ...
  public void setAllowCreate(boolean tags) {
    requireDetached();
    this.tags = tags;
  }
  ...
  public void addOption(String value, String text) {
    ...
    if (options.containsKey(value))
      throw new IllegalArgumentException("Duplicate option: " + value);
  ...
  public void setValue(List<String> value, boolean fireEvents) {
    ...
    if (!multiple && next.size() > 1)
      throw new IllegalArgumentException("Single select accepts one value");
    if (next.contains(null) || new HashSet<>(next).size() != next.size())
      throw new IllegalArgumentException("Invalid selection");
    for (String item : next) {
      if (!options.containsKey(item) && !tags)
        throw new IllegalArgumentException("Add the option before selecting: " + item);
    }
  ...
  @Override
  protected void whenReady(Runnable action) {
    SelectJs.whenReady(action);
  }

  @Override
  protected SelectJs.Handle createPlugin() {
    return SelectJs.create(
        getElement(), tags, placeholder, data -> ValueChangeEvent.fire(this, getValue()));
  }
    ...
    if (provider != null)
      SelectJs.remote(
          ...
          request -> {
            ...
            AsyncCallback<List<SelectOption>> callback =
                new AsyncCallback<List<SelectOption>>() {
                  private boolean completed;

                  @Override
                  public void onSuccess(List<SelectOption> options) {
            ...
            try {
              provider.load(SelectJs.requestQuery(request), callback);
