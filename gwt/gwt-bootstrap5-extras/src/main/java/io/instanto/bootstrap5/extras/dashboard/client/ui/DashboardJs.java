...
final class DashboardJs {
  ...
  static void whenReady(Runnable action) {
    DashboardResources.whenReady(DashboardJs::isReady, action);
  }
  ...
  static void add(GridStack p, Element el) {
    p.makeWidget(el);
  }

  static void remove(GridStack p, Element el) {
    p.removeWidget(el, false);
  }

  static String layout(GridStack p) {
    return NativeJson.stringify(p.save(false));
  }

  static void restore(GridStack p, String json) {
    p.load(NativeJson.parse(json), false);
  }

  static void enabled(GridStack p, boolean enabled) {
    p.setStatic(!enabled);
  }

  static void destroy(GridStack p) {
    p.destroy(false);
  }
