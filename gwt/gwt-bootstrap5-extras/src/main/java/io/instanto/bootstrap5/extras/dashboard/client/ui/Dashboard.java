...
public class Dashboard extends PluginWidget<DashboardJs.GridStack>
    implements HasEnabled, HasValueChangeHandlers<String> {
  ...
  public void add(Widget child) {
    if (!(child instanceof DashboardTile))
      throw new IllegalArgumentException("Expected a DashboardTile");
  ...
  public String getLayoutJson() {
    return isReady() ? DashboardJs.layout(plugin()) : layout;
  }
  ...
  protected void whenReady(Runnable action) {
    DashboardJs.whenReady(action);
  }

  @Override
  protected DashboardJs.GridStack createPlugin() {
    return DashboardJs.create(
        getElement(),
        enabled,
        value -> {
          layout = value;
  ...
  protected void afterCreate() {
    if (layout != null) DashboardJs.restore(plugin(), layout);
  }
  ...
  @Override
  protected void destroyPlugin(DashboardJs.GridStack plugin) {
    DashboardJs.destroy(plugin);
  }
}
