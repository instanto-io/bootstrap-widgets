...
public class DataTable extends PluginWidget<GridJs.Handle>
  ...
  public void setRows(List<? extends Map<String, ?>> rows) {
    requireDetached();
    this.rows = Json.encode(rows);
  }

  /** Current rows, including edits. Tree children use Tabulator's _children field. */
  public String getRowsJson() {
    return isReady() ? GridJs.data(plugin()) : rows;
  }

  public void setGroupBy(String field) {
    requireDetached();
    group = field == null ? "" : field;
  }
  ...
  public void setPageSize(int size) {
    requireDetached();
    if (size < 0) throw new IllegalArgumentException("Negative page size");
    pageSize = size;
  }
  ...
  protected void whenReady(Runnable action) {
    GridJs.whenReady(action);
  }

  @Override
  protected GridJs.Handle createPlugin() {
    return GridJs.create(
        getElement(),
        columns,
        rows,
        group,
  ...
  @Override
  protected void destroyPlugin(GridJs.Handle plugin) {
    GridJs.destroy(plugin);
  }
}
