...
final class GalleryJs {
    ...
    static final class Handle {
        ...
        private Handle(Lightbox lightbox) { this.lightbox = lightbox; }
    }

    static void whenReady(Runnable action) { GalleryResources.whenReady(GalleryJs::isReady, action); }

    static boolean isReady() { return Js.global().get("PhotoSwipeLightbox") != null; }
    ...
    static Handle create(Element el, PluginCallback changed) {
        ...
        p.lightbox.on("change", () -> changed.onEvent(String.valueOf(p.lightbox.getPswp().getCurrIndex())));
        p.lightbox.on("openingAnimationEnd", () -> {
    ...
    static void destroy(Handle p) { p.lightbox.destroy(); }
    ...
    static class Lightbox {
        ...
        native void destroy();
        @JsProperty(name = "pswp") native Viewer getPswp();
    }
}
