...
final class SelectJs {
  ...
  static void whenReady(Runnable action) {
    SelectResources.whenReady(SelectJs::isReady, action);
  }

  static boolean isReady() {
    return Js.global().get("TomSelect") != null;
  }
  ...
  static void setValues(Handle p, String json) {
    p.select.setValue(NativeJson.parse(json), true);
  }

  static String values(Handle p) {
    return NativeJson.stringify(p.select.getItems());
  }

  static int count(Handle p) {
    return p.select.getItems().getLength();
  }

  static String item(Handle p, int i) {
    return Js.asString(p.select.getItems().getAt(i));
  }
    ...
    p.select
        ...
        .setLoad(
            (query, callback) -> {
              ...
              load.onEvent(
                  NativeJson.stringify(JsPropertyMap.<Object>of("id", id, "query", query)));
            });
  }

  static String requestId(String json) {
    return Js.asString(Js.asPropertyMap(NativeJson.parse(json)).get("id"));
  }

  static String requestQuery(String json) {
    return Js.asString(Js.asPropertyMap(NativeJson.parse(json)).get("query"));
  }

  static void complete(Handle p, String id, String json) {
    LoadCallback callback = p.callbacks.remove(id);
    if (callback != null)
      callback.accept(Js.asAny(json == null ? Js.undefined() : NativeJson.parse(json)));
  }
  ...
  interface LoadCallback {
    void accept(Any results);
  }

  @JsFunction
  interface Loader {
    void load(String query, LoadCallback callback);
  }

  @JsType(isNative = true)
  interface NativeElement {
    Any querySelector(String selectors);
  }

  /**
   * The settings that take callbacks. Typed properties, not JsPropertyMap.set, because TeaVM makes
   * a lambda a JavaScript function only where its target type says so.
   */
  @JsType(isNative = true)
  interface Settings {
    @JsProperty
    void setOnChange(Listener listener);

    @JsProperty
    void setLoad(Loader loader);
  }
  ...
  static class TomSelect {
    ...
    native JsArrayLike<Object> getItems();

    @JsProperty(name = "settings")
    native Settings getSettings();
