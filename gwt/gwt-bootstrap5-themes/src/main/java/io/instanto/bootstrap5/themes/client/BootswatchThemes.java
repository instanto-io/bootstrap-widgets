...
/**
 * The Bootswatch 5.3.8 stylesheets vendored by this module, as a catalogue of {@link Theme}
 * instances ready to hand to {@link
 * io.instanto.bootstrap5.client.ui.theme.Themes#register(Iterable)}.
 *
 * <p>Each is a complete replacement for bootstrap.css rather than an overlay, so exactly one is
 * active at a time. The dark flag is not a guess: it was derived from each stylesheet's own {@code
 * --bs-body-bg} and {@code --bs-body-color}, a theme counting as dark when its text is lighter than
 * its ground. {@link io.instanto.bootstrap5.client.ui.theme.Themes} uses it to set {@code
 * data-bs-theme}, which is what Bootstrap 5.3's own components read.
 *
 * <p>Bootswatch is MIT licensed; see BOOTSWATCH-LICENSE alongside these files.
 */
public enum BootswatchThemes {
  ...
  Theme at(final String basePath) {
    return new Theme(
        getThemeName(),
        displayName,
        basePath + "bootswatch-" + getThemeName() + "-" + VERSION + ".min.cache.css",
        dark);
  }
