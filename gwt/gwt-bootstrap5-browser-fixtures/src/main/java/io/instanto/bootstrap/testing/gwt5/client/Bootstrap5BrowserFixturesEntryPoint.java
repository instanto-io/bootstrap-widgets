...
public final class Bootstrap5BrowserFixturesEntryPoint implements EntryPoint {
  ...
  private Widget radioFixture() {
    ...
    RadioButton first =
        tagged(new RadioButton("touch-radio-fixture", "First"), "behaviour/radio-button/first");
    RadioButton second =
        tagged(new RadioButton("touch-radio-fixture", "Second"), "behaviour/radio-button/second");
  ...
  private Widget alignedDropdownFixture() {
    ...
    Button toggle =
        tagged(new Button("Constrained menu"), "behaviour/dropdown/aligned-width-toggle");
    toggle.setDataToggle(Toggle.DROPDOWN);
    toggle.setWidth("100%");
    DropDownMenu menu = tagged(new DropDownMenu(), "behaviour/dropdown/aligned-width-menu");
    menu.setWidth("100%");
  ...
  private Widget checkboxLabelFixture() {
    ...
    checkbox.addValueChangeHandler(
        event -> {
          ...
          checkbox
              .getElement()
              .setAttribute("data-source-match", Boolean.toString(event.getSource() == checkbox));
        });
    return fixture("Checkbox label", checkbox);
  }

  private Widget radioLabelFixture() {
    Radio radio = tagged(new Radio("form-radio-label", "Select"), "behaviour/form/radio-label");
    initializeCounter(radio.getElement(), CHANGE_COUNT);
    initializeValue(radio, false);
    radio.addValueChangeHandler(
        event -> {
          increment(radio.getElement(), CHANGE_COUNT);
          initializeValue(radio, event.getValue());
          radio
              .getElement()
              .setAttribute("data-source-match", Boolean.toString(event.getSource() == radio));
        });
  ...
  private Widget textLabelFixture() {
    ...
    FormLabel label = tagged(new FormLabel("Account name"), "behaviour/form/text-label/label");
  ...
  private Widget textValueFixture() {
    ...
    textBox.addValueChangeHandler(
        event -> {
          ...
          textBox
              .getElement()
              .setAttribute("data-source-match", Boolean.toString(event.getSource() == textBox));
        });
  ...
  private Widget validationFixture() {
    ...
    validate.addClickHandler(
        event ->
            group
                .getElement()
                .setAttribute("data-validation-result", Boolean.toString(textBox.validate(true))));
  ...
  private Widget formSubmissionFixture() {
    ...
    form.addSubmitHandler(
        event -> {
          ...
          form.getElement()
              .setAttribute("data-source-match", Boolean.toString(event.getSource() == form));
          event.cancel();
        });
  ...
  private Widget listSelectionFixture() {
    ...
    listBox.addChangeHandler(
        event -> {
          ...
          listBox
              .getElement()
              .setAttribute("data-value", listBox.getItemText(listBox.getSelectedIndex()));
          listBox
              .getElement()
              .setAttribute("data-source-match", Boolean.toString(event.getSource() == listBox));
        });
  ...
  private Widget radioGroupFixture() {
    ...
    Radio first =
        tagged(new Radio("form-radio-group", "First"), "behaviour/form/radio-group/first");
    first.setFormValue("first");
    Radio second =
        tagged(new Radio("form-radio-group", "Second"), "behaviour/form/radio-group/second");
  ...
  private Widget accordionItem(String name, boolean open) {
    ...
    PanelCollapse collapse =
        tagged(new PanelCollapse(), "behaviour/collapse/accordion/" + name + "-panel");
    collapse.getElement().setId(collapseId);
    collapse.setIn(open);
    collapse.setDataParent("#behaviour-collapse-accordion");
  ...
  private Widget detachedCollapseFixture() {
    ...
    Button remount = tagged(new Button("Remount collapse"), "behaviour/collapse/detach/remount");
    remount.addClickHandler(
        event -> {
    ...
    Button toggle =
        tagged(new Button("Open remounted collapse"), "behaviour/collapse/detach/toggle");
    toggle.setDataToggle(Toggle.COLLAPSE);
  ...
  private Widget basicLifecycleFixture() {
    ...
    Button widget = tagged(new Button("Lifecycle widget"), "behaviour/lifecycle/basic/widget");
    widget.addAttachHandler(event -> recordLifecycle(state, host, widget));
    host.add(widget);
    Button remove = tagged(new Button("Remove widget"), "behaviour/lifecycle/basic/remove");
  ...
  private Widget remountLifecycleFixture() {
    ...
    Button widget = tagged(new Button("Remountable widget"), "behaviour/lifecycle/remount/widget");
    ...
    oldHost.add(widget);
    Button remount = tagged(new Button("Move to fresh host"), "behaviour/lifecycle/remount/action");
    remount.addClickHandler(
        event -> {
          ...
          state
              .getElement()
              .setAttribute("data-parent-match", Boolean.toString(widget.getParent() == freshHost));
          state
              .getElement()
              .setAttribute(
                  "data-new-attach-count",
                  Integer.toString(
                      Integer.parseInt(widget.getElement().getAttribute("data-attach-count")) - 1));
        });
    state.add(remount);
    state.add(oldHost);
    state.add(freshHost);
  ...
  private Widget handlerLifecycleFixture() {
    ...
    Button widget =
        tagged(new Button("Activate after remounts"), "behaviour/lifecycle/handlers/widget");
    countClicks(widget, widget);
    firstHost.add(widget);
    final FlowPanel[] currentHost = {firstHost};
    Button remount =
        tagged(new Button("Remount three times"), "behaviour/lifecycle/handlers/remount");
    remount.addClickHandler(
        event -> {
          for (int i = 0; i < 3; i++) {
          ...
          state
              .getElement()
              .setAttribute("data-current-host", currentHost[0] == firstHost ? "first" : "second");
        });
    state.add(remount);
    state.add(firstHost);
  ...
  private void recordLifecycle(FlowPanel state, FlowPanel host, Widget widget) {
    state
        .getElement()
        .setAttribute("data-parent-match", Boolean.toString(widget.getParent() == host));
  ...
  private Widget textInputGroupFixture() {
    ...
    InputGroupAddon prefix =
        tagged(new InputGroupAddon(), "behaviour/input-group/text-addons/prefix");
    ...
    InputGroupAddon suffix =
        tagged(new InputGroupAddon(), "behaviour/input-group/text-addons/suffix");
  ...
  private Widget buttonInputGroupFixture() {
    ...
    InputGroupButton addon =
        tagged(new InputGroupButton(), "behaviour/input-group/button-addon/container");
  ...
  private Widget suggestBoxLifecycleFixture() {
    ...
    SuggestBox suggestBox =
        tagged(new SuggestBox(countryOracle()), "behaviour/suggest-box/lifecycle/control");
    recordSuggestions(suggestBox);
    host.add(suggestBox);
  ...
  private void recordSuggestions(SuggestBox suggestBox) {
    ...
    suggestBox.addSelectionHandler(
        event -> {
          ...
          suggestBox
              .getElement()
              .setAttribute("data-selected-value", event.getSelectedItem().getReplacementString());
          suggestBox
              .getElement()
              .setAttribute("data-source-match", Boolean.toString(event.getSource() == suggestBox));
        });
  }
  ...
  private Widget themeSwitcherFixture() {
    ...
    Button standard = tagged(new Button("Bootstrap"), "behaviour/themes/switcher/standard");
    standard.addClickHandler(
        event -> {
          Themes.apply(StandardThemes.BOOTSTRAP);
          recordTheme(state);
        });
    Button flatly = tagged(new Button("Flatly"), "behaviour/themes/switcher/flatly");
    flatly.addClickHandler(
        event -> {
          Themes.apply("flatly");
          recordTheme(state);
        });
  ...
  private void recordTheme(FlowPanel state) {
    ...
    state.getElement().setAttribute("data-current-theme", current == null ? "" : current.getName());
    state.getElement().setAttribute("data-dark", Boolean.toString(Themes.isDark()));
    state
        .getElement()
        .setAttribute("data-theme-url", link == null ? "" : link.getAttribute("href"));
  }
  ...
  private Widget keyboardModalFixture() {
    ...
    Button target = tagged(new Button("Open keyboard modal"), "behaviour/modal/keyboard/target");
    target.setDataToggle(Toggle.MODAL);
    target.setDataTargetWidget(modal);
    state.add(target);
    state.add(modal);
  ...
  private Widget exclusiveModalFixture() {
    ...
    Button openSecond =
        tagged(new Button("Open second modal"), "behaviour/modal/exclusive/open-second");
    ...
    Button target = tagged(new Button("Open first modal"), "behaviour/modal/exclusive/open-first");
    target.setDataToggle(Toggle.MODAL);
  ...
  private Widget tooltipOptionsFixture() {
    Button target = tagged(new Button("Show delayed tooltip"), "behaviour/tooltip/options");
  ...
  private Widget tooltipDisposalFixture() {
    ...
    Button target = tagged(new Button("Disposable tooltip"), "behaviour/tooltip/disposal/target");
    target.getElement().setAttribute("data-event-order", "");
    ...
    Button destroy = tagged(new Button("Destroy and detach"), "behaviour/tooltip/disposal/destroy");
    destroy.addClickHandler(
        event -> {
          tooltip.destroy();
          host.remove(target);
          state.getElement().setAttribute("data-destroyed", "true");
        });
    Button remount = tagged(new Button("Remount"), "behaviour/tooltip/disposal/remount");
  ...
  private Widget popoverProgrammaticFixture() {
    ...
    Button target =
        tagged(new Button("Programmatic popover target"), "behaviour/popover/programmatic/target");
    ...
    Button showHide =
        tagged(new Button("Show then hide"), "behaviour/popover/programmatic/show-hide");
    showHide.addClickHandler(
        event -> {
          popover.show();
          popover.hide();
        });
  ...
  private Widget pluginLifecycleFixture() {
    ...
    Button target = tagged(new Button("Lifecycle tooltip"), "behaviour/lifecycle/plugin/target");
    ...
    Button detach = tagged(new Button("Detach tooltip"), "behaviour/lifecycle/plugin/detach");
    detach.addClickHandler(event -> host.remove(target));
    Button remount = tagged(new Button("Remount tooltip"), "behaviour/lifecycle/plugin/remount");
    remount.addClickHandler(
        event -> {
          host.add(target);
          tooltip.show();
  ...
  private Widget basicTabsFixture() {
    return fixture(
        "Basic tabs",
        createTabSet("basic-tabs", "behaviour/tabs/basic", false, false, TabPosition.TOP, false));
  }
  ...
  private Widget disabledTabsFixture() {
    return fixture(
        "Disabled tabs",
        createTabSet(
            "disabled-tabs", "behaviour/tabs/disabled", true, false, TabPosition.TOP, false));
  }
  ...
  private Widget fadingTabsFixture() {
    return fixture(
        "Fading tabs",
        createTabSet("fading-tabs", "behaviour/tabs/fade", false, true, TabPosition.TOP, false));
  }
  ...
  private Widget positionedTabsFixture() {
    ...
    positions.add(
        createTabSet(
            "left-tabs", "behaviour/tabs/positions/left", false, false, TabPosition.LEFT, false));
    positions.add(
        createTabSet(
            "right-tabs",
            "behaviour/tabs/positions/right",
            false,
            false,
            TabPosition.RIGHT,
            false));
    positions.add(
        createTabSet(
            "below-tabs",
            "behaviour/tabs/positions/below",
            false,
            false,
            TabPosition.BELOW,
            false));
  ...
  private Widget createTabSet(
      String prefix,
      String fixtureId,
      boolean disableThird,
      boolean fade,
    ...
    if (programmatic) {
      Button showSecond = tagged(new Button("Show second tab"), fixtureId + "/show-second");
  ...
  private TabListItem createTab(
      String prefix, String fixtureId, String name, boolean active, boolean enabled) {
  ...
  private TabPane createTabPane(
      String prefix, String fixtureId, String name, boolean active, boolean fade) {
  ...
  private Widget programmaticCheckboxFixture() {
    ...
    CheckBoxButton button =
        tagged(new CheckBoxButton("Programmatic"), "behaviour/check-box-button/basic");
    initializeCounter(button.getElement(), CHANGE_COUNT);
    initializeValue(button, false);
    button.addValueChangeHandler(
        event -> {
    ...
    Button silent =
        tagged(new Button("Set true silently"), "behaviour/check-box-button/set-silent");
    silent.addClickHandler(
        event -> {
    ...
    Button firing =
        tagged(new Button("Set false with event"), "behaviour/check-box-button/set-firing");
    firing.addClickHandler(
        event -> {
  ...
  private Widget buttonGroupSizesFixture() {
    ...
    Button change =
        tagged(new Button("Use small group size"), "behaviour/button-group/sizes/change");
    change.addClickHandler(
        event -> {
  ...
  private Widget verticalButtonGroupFixture() {
    VerticalButtonGroup group =
        tagged(new VerticalButtonGroup(), "behaviour/button-group/vertical");
    group.add(new Button("First"));
    group.add(new Button("Second"));
    group.add(new Button("Third"));
  ...
  private Widget nestedButtonGroupFixture() {
    ...
    Button sibling =
        tagged(new Button("Sibling"), "behaviour/button-group/nested-dropdown/sibling");
    countClicks(sibling, sibling);
    Button toggle = tagged(new Button("More"), "behaviour/button-group/nested-dropdown/toggle");
    toggle.addStyleName("dropdown-toggle");
    toggle.setDataToggle(Toggle.DROPDOWN);
    DropDownMenu menu = tagged(new DropDownMenu(), "behaviour/button-group/nested-dropdown/menu");
    menu.add(new DropDownItem("Nested action", "#"));
    group.add(sibling);
    group.add(toggle);
    group.add(menu);
    ...
    remove.addClickHandler(
        event -> {
          ...
          group
              .getElement()
              .setAttribute(
                  "data-removed-parent-null", Boolean.toString(middle.getParent() == null));
        });
    container.add(group);
  ...
  private void initializeValue(
      com.google.gwt.user.client.ui.HasValue<Boolean> widget, boolean value) {
