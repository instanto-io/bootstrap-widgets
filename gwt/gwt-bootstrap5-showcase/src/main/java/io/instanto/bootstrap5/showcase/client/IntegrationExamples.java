...
final class IntegrationExamples {
  ...
  static Widget selects() {
    ...
    examples.add(
        new Paragraph(
            "Skills: type to search, press Enter to create a tag, or remove a selection."));
    examples.add(tags);
    examples.add(status);
    ...
    examples.add(
        new Paragraph(
            "Asynchronous options: this example simulates a directory lookup with a short delay."));
    examples.add(remote);
    return examples;
  }
  ...
  static Widget grid() {
    ...
    table.setColumns(
        Arrays.asList(
            name, new GridColumn("Team", "team"), new GridColumn("Location", "location")));
    ...
    table.addValueChangeHandler(
        event -> status.setText("Changes captured in the table's row data."));
    examples.add(status);
    examples.add(table);
  ...
  static Widget sortable() {
    ...
    for (String title : Arrays.asList("To do", "Reviewed")) {
      ...
      list.addValueChangeHandler(
          event -> status.setText(title + ": " + event.getValue().size() + " cards"));
      column.add(list);
      board.add(column);
    }
    examples.add(status);
  ...
  static Widget dashboard() {
    ...
    save.addClickHandler(
        event -> {
          saved[0] = dashboard.getLayoutJson();
          status.setText("Layout saved for this session.");
        });
    Button restore = new Button("Restore layout");
    restore.addClickHandler(
        event -> {
          if (saved[0] != null) {
            dashboard.setLayoutJson(saved[0]);
            status.setText("Saved layout restored.");
          }
        });
  ...
  static Widget gallery() {
    ...
    Paragraph status =
        new Paragraph("Select a thumbnail. Swipe between images, zoom in, or use the arrow keys.");
    gallery.addSelectionHandler(
        event -> status.setText("Viewing image " + (event.getSelectedItem() + 1)));
    examples.add(status);
