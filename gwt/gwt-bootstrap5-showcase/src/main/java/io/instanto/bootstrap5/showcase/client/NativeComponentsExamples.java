...
final class NativeComponentsExamples {
    ...
    add.addClickHandler(
        event ->
            notifications.notify(
                "Saved " + ++count[0],
                "Each notification closes independently after five seconds.",
                5000));
    controls.add(show);
    controls.add(hide);
    controls.add(add);
  ...
  static Widget offcanvas() {
    ...
    controls.setStyleName("d-flex flex-wrap gap-2 mb-3");
    InlineLabel status =
        new InlineLabel("Choose a panel. Escape, the backdrop or Close dismisses it.");
    for (OffcanvasPlacement placement : OffcanvasPlacement.values()) {
      ...
      body.add(
          new Paragraph(
              "This panel uses Bootstrap's backdrop, focus management and keyboard behaviour."));
      TextBox search = new TextBox();
      search.setPlaceholder("Filter records");
      search.getElement().setAttribute("aria-label", "Filter records");
      body.add(search);
      drawer.add(body);
      drawer.addShownHandler(
          event -> status.setText("Panel shown: " + placement.name().toLowerCase()));
      drawer.addHiddenHandler(
          event -> status.setText("Panel hidden; focus returned to the opening button."));
    ...
    scrolling.add(
        new OffcanvasBody("No backdrop. Scroll the page or close this panel with Escape."));
