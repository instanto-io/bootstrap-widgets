...
public abstract class AbstractButton extends ComplexWidget
  ...
  private final DataTargetMixin<AbstractButton> targetMixin =
      new DataTargetMixin<AbstractButton>(this);
  ...
  private final FocusableMixin<AbstractButton> focusableMixin =
      new FocusableMixin<AbstractButton>(this);
   ...
   *
   * <p>If button is inside a {@link io.instanto.bootstrap5.client.ui.Modal} and dismiss type is
   * {@code MODAL} the button will act as the dismiss (close) button for this Modal. Same goes for
   * {@link io.instanto.bootstrap5.client.ui.Alert} and dismiss type {@code ALERT}.
   *
  ...
  /**
   * Bootstrap 5 names button colours btn-primary .. btn-dark plus btn-link, with a btn-outline-*
   * counterpart for each. {@link Variant} is that vocabulary; {@link ButtonType} is the Bootstrap 3
   * spelling kept for source compatibility, and setting one keeps the other in step.
   */
