 ...
 * reach no JavaScript of their own. The calls go through JsInterop, so GWT and TeaVM compile
 * this one source.</p>
 *
...
public final class BootstrapComponent {
    ...
    public static void createCarousel(final Element element, final int interval, final String pause,
            final boolean wrap) {
     ...
     */
    public static void createPlugin(final Element element, final String component, final boolean animation,
            final boolean html, final String placement, final String trigger, final int showDelay, final int hideDelay,
            final String container, final String selector, final String boundary, final int padding, final String title,
            final String template, final String content) {
        final Component type = component(component);
        if (type == null) {
            return;
        }
        ...
        options.set("delay", JsPropertyMap.<Object>of("show", (double) showDelay, "hide", (double) hideDelay));
    ...
    /**
     * Creates the component with options, as {@link #create(Element, String)} does, but
     * taking {@code toggle: false} -- which Collapse needs so that constructing it does not
     * immediately toggle the panel.
     */
    ...
    public static void callCollapse(final Element element, final String method) {
        final Component type = component("Collapse");
        if (type != null) {
            invoke(type.getOrCreateInstance(Js.asAny(element), JsPropertyMap.<Object>of("toggle", false)), method);
        }
    }
    ...
    public static void rememberFocus(final Element el) {
        ...
        slot.setListener(event -> {
            ...
            if (Js.isTruthy(element.get("isConnected")) && "function".equals(Js.typeof(element.get("focus")))) {
    ...
    private static void construct(final Component type, final Element element, final JsPropertyMap<Object> options) {
    ...
    private static void finishTransition(final Element el) {
        final Object window = Js.asPropertyMap(Js.<Node>uncheckedCast(Js.asAny(el)).getOwnerDocument()).get("defaultView");
        final Any event = Reflect.construct(Js.asAny(Js.asPropertyMap(window).get("Event")), JsArrays.of("transitionend"));
