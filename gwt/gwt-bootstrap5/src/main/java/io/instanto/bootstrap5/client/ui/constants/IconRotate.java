 ...
 * Icon rotation.
 *
 * <p>Neither Bootstrap 5 nor Bootstrap Icons defines rotation utilities, so these classes are
 * declared by this library in {@code css/gwt-bootstrap5.cache.css}. They are namespaced {@code
 * gbm-icon-*} rather than {@code bi-*} so that they cannot collide with an icon name — {@code
 * bi-border}, for one, is a real Bootstrap icon.
 */
