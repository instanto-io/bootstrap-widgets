...
public class FormGroup extends ElementPanel implements HasSize<FormGroupSize>, HasValidationState {
  ...
  /**
   * Bootstrap 3 put .has-error on the group and let descendant selectors colour everything inside
   * it. Bootstrap 5 puts .is-invalid on the control itself, so that is where the state goes; the
   * group keeps the property so calling code and the error handler do not have to know the
   * difference.
   */
  @Override
