 ...
 *
 * <p>Bootstrap 3 styled a select with {@code form-control}; Bootstrap 5 gives selects their own
 * {@code form-select} class, which is what the underlying {@link ListBox} applies. Validation puts
 * {@code is-invalid} on the select and shows the message from a sibling {@code invalid-feedback}
 * element.
 *
...
public class ValueListBox<T> extends com.google.gwt.user.client.ui.ValueListBox<T>
  ...
  /**
   * Kept for parity with the Bootstrap 3 widget. A select has no placeholder attribute in HTML, so
   * this sets it without visible effect; Bootstrap 5 spells the same intent as a disabled, selected
   * first option.
   */
  @Override
