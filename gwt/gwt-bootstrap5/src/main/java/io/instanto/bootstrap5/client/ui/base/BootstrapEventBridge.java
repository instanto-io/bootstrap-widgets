 ...
 *
 * <p>Reached through JsInterop, so GWT and TeaVM compile this one source. Each element keeps
 * its listeners on a property, one per event name, so binding again replaces the listener and
 * unbinding removes the exact function that was added.</p>
 */
public final class BootstrapEventBridge {
    ...
    public static void bind(final Element element, final String eventName, final BootstrapEventHandler handler) {
    ...
    public static void unbindAll(final Element element) {
        ...
        listeners.forEach(eventName -> target.removeEventListener(eventName, Js.uncheckedCast(listeners.get(eventName))));
    ...
    /**
     * The listener as the JavaScript function the DOM holds. Passing it through a typed
     * property turns the lambda into a function once, so the same function is added,
     * stored and later removed; TeaVM would otherwise convert it afresh on each pass.
     */
    ...
    /**
     * Views a JavaScript event as GWT's NativeEvent. {@code Object.assign} with no sources
     * returns its target unchanged; declaring its result as NativeEvent lets the TeaVM
     * boundary wrap the event, while on GWT it is the event itself.
     */
