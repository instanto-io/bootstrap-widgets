...
/**
 * Button representing a radio button used within a {@link ButtonGroup} that has toggle set to
 * {@code Toogle.BUTTONS}.
 *
 * <p>If you are looking for a classic radio button see {@link RadioButton}.
 *
 * @author Sven Jacobs
 */
public class RadioButton extends Radio
   ...
   * set.
   *
   * <p>Radio buttons are grouped by their name attribute, so changing their name using the
   * setName() method will also change their associated group.
   *
  ...
  /**
   * Creates a new radio button associated with a particular group, and initialized with the given
   * label (optionally treated as HTML). All radio buttons associated with the same group name
  ...
  private static String buttonStyle(ButtonType type, boolean outline) {
    ...
    return outline && type != ButtonType.LINK
        ? "btn-outline-" + cssName.substring("btn-".length())
        : cssName;
  }
   ...
   *
   * <p>Bootstrap 5 styles {@code .btn.active} with the very same rule as {@code .btn-check:checked
   * + .btn}, so this changes nothing about how the button looks. What it changes is how the browser
   * finds out. Relying on the sibling combinator alone leaves the label's style dependent on a
   * state change on another element, and WebKit does not reliably invalidate that on touch: a
   * toggle would stay looking pressed after being switched off until a scroll forced a repaint.
   * Setting a class on the label is a direct mutation, which always invalidates.
   *
   * <p>This is what GwtBootstrap3 did through ActiveMixin; it was lost when the widget moved to the
   * btn-check markup.
   */
  private void updatePressedState() {
    ...
    final NodeList<com.google.gwt.dom.client.Element> radios =
        Document.get().getBody().getElementsByTagName("input");
