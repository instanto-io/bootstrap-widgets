...
/**
 * The Bootstrap Icons vocabulary: one constant per icon in Bootstrap Icons 1.13.1 (2078 icons).
 * Each constant emits that icon's own {@code bi-*} class, which the {@link
 * io.instanto.bootstrap5.client.ui.Icon} widget pairs with the {@code bi} base class.
 *
 * <p>This replaces the Font Awesome 4 vocabulary GwtBootstrap3 carried. Bootstrap 3 shipped Font
 * Awesome; Bootstrap 5 ships Bootstrap Icons, and the two name their icons differently, so a Font
 * Awesome name is not a Bootstrap Icons name. Around 200 Font Awesome 4 names happen to coincide
 * with a Bootstrap Icons name and so are still present below; the rest are gone, and code that used
 * one will not compile until it names a Bootstrap Icons icon. That is deliberate: a missing
 * constant is a compile error a developer can act on, whereas keeping the old constant would render
 * nothing or, worse, an approximation of the icon that was asked for.
 *
 * <p>Names that start with a digit take an {@code N} prefix, because a Java identifier cannot start
 * with one: {@code bi-1-circle} is {@link #N1_CIRCLE}.
 *
 * <p>Generated from the {@code bootstrap-icons} stylesheet in {@code client/resource/css};
