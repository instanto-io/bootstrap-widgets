...
public abstract class AbstractVisibilityWidget extends Div {
  ...
  protected void onLoad() {
    ...
    BootstrapEventBridge.bind(
        getElement(), "show.bs." + eventSuffix, event -> fireEvent(new ShowEvent(event)));
    BootstrapEventBridge.bind(
        getElement(), "shown.bs." + eventSuffix, event -> fireEvent(new ShownEvent(event)));
    BootstrapEventBridge.bind(
        getElement(), "hide.bs." + eventSuffix, event -> fireEvent(new HideEvent(event)));
    BootstrapEventBridge.bind(
        getElement(), "hidden.bs." + eventSuffix, event -> fireEvent(new HiddenEvent(event)));
  }
  ...
  protected void setOption(String name, String value) {
    if (isShown() || hasStyle("hiding")) {
      throw new IllegalStateException(
          "Hide the widget and wait for hidden before changing options");
    }
