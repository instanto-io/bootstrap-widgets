...
public class ComplexWidget extends ComplexPanel
  ...
  protected void insert(
    ...
    if (domInsert) {
      ...
      Widget beforeWidget =
          getChildren().size() > beforeIndex + 1 ? getChildren().get(beforeIndex + 1) : null;
  ...
  /**
   * <b> Overrides the remove(Widget) method to avoid potential NPE when removing a {@link Text}
   * node which is not supposed to be a "widget" in GWT. </b> {@inheritDoc}
   */
  @Override
