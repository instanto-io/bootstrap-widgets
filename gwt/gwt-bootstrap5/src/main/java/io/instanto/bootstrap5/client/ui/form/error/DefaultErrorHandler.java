...
/**
 * This is the default {@link ErrorHandler} implementation. The assumption is that every {@link
 * ValueBoxBase} instance will have a {@link HasValidationState} parent. If there is a {@link
 * HelpBlock} that is a child of the {@link HasValidationState} parent then error messages will be
...
public class DefaultErrorHandler implements ErrorHandler {
  ...
  public DefaultErrorHandler(final Widget widget) {
    ...
    this.inputWidget.addAttachHandler(
        new Handler() {
          @Override
  ...
  public void showErrors(final List<EditorError> errors) {
    ...
    if (validationStateParent != null) {
      validationStateParent.setValidationState(
          hasErrors ? ValidationState.ERROR : ValidationState.NONE);
    }
  ...
  /**
   * Walks up to the nearest {@link HasValidationState} -- in practice the {@link FormGroup} -- and
   * remembers any {@link HelpBlock} already inside it to carry the message.
   */
  public void init() {
    if (initialized) {
      return;
  ...
  /**
   * Bootstrap 5 shows a validation message from a .invalid-feedback element next to the control.
   * Bootstrap 3 code was not obliged to declare one -- without a HelpBlock the message simply went
   * nowhere -- so one is created on demand here and removed again on cleanup.
   */
