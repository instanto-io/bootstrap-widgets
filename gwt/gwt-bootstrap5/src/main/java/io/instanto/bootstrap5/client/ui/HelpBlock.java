...
public class HelpBlock extends ElementPanel implements IsEditor<LeafValueEditor<String>> {
   ...
   *
   * <p>Bootstrap 3 styled these with .help-block inside a .has-error group. Bootstrap 5 uses
   * .invalid-feedback, which is hidden unless it follows a sibling carrying .is-invalid; .d-block
   * is Bootstrap's own escape hatch for showing it when the control is not a direct sibling, which
   * is the case whenever the control is wrapped, as it is in a SuggestBox.
   */
  ...
  /**
   * Bootstrap 3 put a contextual icon inside the help block. Bootstrap 5 has no equivalent
   * component, so the icon is prepended to the text.
   */
