 ...
 *
 * <p>The text box carries {@code form-control} and the suggestion popup is rendered as a Bootstrap
 * 5 {@code dropdown-menu} of {@code dropdown-item} entries, rather than the Bootstrap 3 markup
 * GwtBootstrap3 produced.
 *
 * <p>Every accessor here works on the widget itself rather than on the box it wraps. GWT's
 * SuggestBox is a Composite built with {@code initWidget(box)}, so its element <em>is</em> the
 * {@code <input>}; styling or configuring the composite configures the input. That is how
 * GwtBootstrap3 does it, and it sidesteps {@code getTextBox()}, which casts to {@code
 * com.google.gwt.user.client.ui.TextBoxBase} -- a type this library's {@link TextBox} does not
 * extend, since it descends from the Bootstrap {@link
 * io.instanto.bootstrap5.client.ui.base.ValueBoxBase} instead.
 *
 * <p>Validation follows Bootstrap 5 as well: an invalid box gets {@code is-invalid} and the message
 * is shown in a sibling {@code invalid-feedback} element, which {@link
 * io.instanto.bootstrap5.client.ui.form.error.DefaultErrorHandler} creates if the surrounding
 * {@link FormGroup} has no {@link HelpBlock} of its own.
 */
public class SuggestBox extends com.google.gwt.user.client.ui.SuggestBox
    implements HasId,
        ...
        HasErrorHandler,
        HasValidators<String>,
        HasBlankValidator<String> {

  /**
   * Renders the suggestion popup with the Bootstrap 5 dropdown classes.
   *
   * <p>GwtBootstrap3 positioned and sized the popup by hand from a resize handler. Bootstrap 5
   * dropdowns need neither: the popup is styled as a {@code dropdown-menu} and left to the browser.
   */
  static class BootstrapSuggestionDisplay extends DefaultSuggestionDisplay {
    ...
    protected void showSuggestions(
      ...
      popup
          .getStyle()
          .setWidth(suggestBox.getOffsetWidth(), com.google.gwt.dom.client.Style.Unit.PX);
    }
  }
  ...
  public SuggestBox(
      final SuggestOracle oracle,
      final ValueBoxBase<String> box,
  ...
  public void setSize(final InputSize size) {
    StyleHelper.addUniqueEnumStyleName(
        this, InputSize.class, size == null ? InputSize.DEFAULT : size);
  }
