...
public class CarouselControl extends ElementPanel implements HasHref {
  ...
  /**
   * A control with neither direction nor target set yet, which is what UiBinder needs: it
   * constructs a widget with no arguments and then applies the attributes. Bootstrap 3's
   * CarouselControl has the same no-argument constructor, so a template written against that track
   * carries over.
   */
  ...
  /**
   * Bootstrap 5 addresses the carousel with data-bs-target rather than href, so this sets both and
   * href is kept only for source compatibility.
   */
  @Override
  ...
  /**
   * Bootstrap 5 draws the control with its own carousel-control-*-icon span rather than a glyph
   * from the icon font, so a custom IconType replaces that span's content.
   */
