 ...
 *
 * <p>The counterpart to the Bootstrap 3 track's {@code Themes}: same shape, different mechanism.
 * Bootstrap 5.3 ships light and dark in one stylesheet, so this sets {@code data-bs-theme} on the
 * document element rather than swapping a stylesheet, and needs no extra assets.
 *
 * <p>The choice is remembered in {@code localStorage} where the browser allows it. A browser that
 * refuses storage keeps working, it just does not remember.
 *
 * <pre>{@code
