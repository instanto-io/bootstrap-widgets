...
public final class Bootstrap3BrowserFixturesEntryPoint implements EntryPoint {
  ...
  private Widget dropdownFixture() {
    ...
    menu.add(action);
    AnchorListItem disabled =
        tagged(new AnchorListItem("Disabled"), "behaviour/dropdown/disabled-item");
  ...
  private Widget bootstrapSelectFixture() {
    ...
    select.addValueChangeHandler(
        event -> {
          increment(state.getElement(), CHANGE_COUNT);
          state.getElement().setAttribute("data-value", event.getValue());
          state
              .getElement()
              .setAttribute("data-source-match", Boolean.toString(event.getSource() == select));
        });
    state.add(select);
