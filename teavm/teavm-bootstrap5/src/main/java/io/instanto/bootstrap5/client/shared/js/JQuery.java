 ...
 *
 * <p>The GWT class is a JsInterop native type, which TeaVM cannot compile, and Bootstrap 5 dropped
 * its jQuery dependency — so the plugin entry points here map to Bootstrap 5's own JavaScript API
 * where an equivalent exists, and to a no-op where Bootstrap 5 removed the concept.
 *
 * <p>This file exists because two call sites in the shared source still route through jQuery:
 * {@code HTMLMixin.setHTML} and {@code AbstractButton}'s button-state call. Both are Bootstrap 3
 * leftovers; once they are replaced on the GWT side this class can go.
 */
public class JQuery {
