 ...
 *
 * <p>On GWT this class does not exist and is not needed: the module declares its stylesheets with
 * {@code <stylesheet src="..."/>} in {@code NoThemeResources.gwt.xml} and the compiler injects
 * them, so an application never writes a {@code <link>} for them. TeaVM has no module system to do
 * that, and the alternative -- asking every host page to hand-wire the library's internals -- leaks
 * mechanics the widgets exist to hide. So the TeaVM backend injects the same set itself, from
 * {@link Bootstrap5#initialise()}.
 *
 * <p>The default Bootstrap theme is installed unless the page has already selected one. The icon
 * font and the library's own rules are then layered on top. {@code Themes} owns the theme link and
 * can replace its target later.
 */
public final class Bootstrap5Resources {
  ...
  /**
   * Where everything this module ships is served from. The TeaVM build puts its stylesheets, fonts
   * and vendored scripts under one directory, so a single setting covers the library and every
   * extra: an application says where the module was deployed and nothing else has to be told
   * anything.
   */
  ...
  /**
   * Sets where this module's assets were deployed, which must be called before the first
   * initialisation to have any effect. Everything else is derived from it, so an extra never needs
  ...
  /**
   * Injects a script once, by URL rather than by embedding it. GWT's ClientBundle inlines the
   * vendored JavaScript into the compiled output; TeaVM has no generator to do that, and loading by
   * URL keeps 74KB of parser and sanitiser out of the module. Returns immediately if a script with
  ...
  /**
   * Injects a stylesheet once, for an extra that has one. The library's own sheets go through
   * {@link #ensureInjected()}; this is for modules loaded on demand.
   */
