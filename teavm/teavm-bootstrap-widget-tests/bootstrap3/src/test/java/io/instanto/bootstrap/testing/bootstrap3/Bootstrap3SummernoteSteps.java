...
public class Bootstrap3SummernoteSteps {
  ...
  public void prepare() {
    ...
    Dom.container().setAttribute("id", "summernote-test-host");
    host = RootPanel.get("summernote-test-host");
  }
  ...
  private HTMLElement editable() {
    return Dom.container().querySelector(".note-editable");
  }

  private void create() {
    editor = new Summernote();
    editor.setCode("<p>Hello <b>world</b></p>");
    editor.addSummernoteInitHandler(e -> initialized++);
    editor.addSummernoteChangeHandler(
        e -> {
          changes++;
          source = e.getSource();
        });
  ...
  private void attach() {
    host.add(editor);
    Dom.waitFor(() -> assertNotNull(editable()));
  }
  ...
  public void html() {
    assertTrue(editor.getCode().contains("<b>world</b>"));
    assertEquals(1, initialized);
  }

  @When("its HTML is replaced and then cleared")
  public void replace() {
    editor.setCode("<p>Replacement</p>");
    assertTrue(editor.getCode().contains("Replacement"));
    assertFalse(editor.getCode().contains("world"));
    editor.clear();
  }
  ...
  public void disable() {
    ...
    assertEquals("false", editable().getAttribute("contenteditable"));
  ...
  public void enabled() {
    ...
    assertEquals("true", editable().getAttribute("contenteditable"));
  }
  ...
  public void retained() {
    assertTrue(editor.getCode().contains("world"));
    assertEquals(1, Dom.container().querySelectorAll(".note-editor").getLength());
  ...
  public void toolbar() {
    create();
    editor.setToolbar(new Toolbar().addGroup(ToolbarButton.BOLD, ToolbarButton.CODE_VIEW));
    attach();
  ...
  public void codeview() {
    Dom.click(Dom.container().querySelector("button.btn-codeview"));
  }
  ...
  public void source() {
    ...
    assertTrue(
        ((org.teavm.jso.dom.html.HTMLTextAreaElement)
                Dom.container().querySelector(".note-codable"))
            .getValue()
            .contains("world"));
  }
  ...
  public void french() {
    create();
    editor.setLanguage(SummernoteLanguage.FR_FR);
    attach();
  }

  @When("its height is reconfigured")
  public void reconfigure() {
    editor.setDefaultHeight(180);
    editor.reconfigure();
  }
  ...
  public void configured() {
    ...
    assertTrue(
        Dom.container()
            .querySelector("button.note-btn-bold")
