 ...
 *
 * <p>Bootstrap 3 is the reference for these behaviours: the same feature file drives the Bootstrap
 * 5 steps, so anything asserted here is what the port has to reproduce. The assertions are
 * therefore about what the widgets do -- addon order, independent activation, one reported size --
 * rather than about markup only this generation emits.
 *
 * <p>Layout assertions use browser geometry after the fixture is mounted. This keeps the scenario
 * at widget level while verifying that the packaged stylesheet makes the segments adjoin, share a
 * height, and consume the complete group width.
 */
@CucumberSuite(value = {"features/input-groups.feature"})
public class Bootstrap3InputGroupSteps {
   ...
   *
   * <p>The same two steps the showcase takes before it builds anything: initialise the library,
   * then add through {@code RootPanel}. Reaching straight for the panel would exercise widgets
   * without the resources and plugin checks an application receives.
   */
  @BeforeScenario
  ...
  public void prefixPrecedesControl() {
    ...
    assertTrue(
        "the prefix addon does not precede the control in the DOM",
  ...
  public void suffixFollowsControl() {
    ...
    assertTrue(
        "the suffix addon does not follow the control in the DOM",
  ...
  public void controlConsumesRemainingWidth() {
    double remaining = width(element(group)) - width(element(prefix)) - width(element(suffix));
    double actual = width(element(control));
    assertTrue(
        "control width " + actual + "px differs from remaining width " + remaining + "px",
        Math.abs(actual - remaining) <= 1);
  }
   ...
   *
   * <p>Read from the element's own children rather than through a document scope, because ING-001
   * asserts this on a group that has deliberately not been mounted: addon order is a property of
   * the widget, not of having been attached.
   */
  ...
  private static void assertAdjoins(HTMLElement left, HTMLElement right) {
    ...
    assertTrue(
        "a gap of " + gap + "px separates two segments of one control", Math.abs(gap) <= 1.0);
  }
  ...
  private static void assertSameVerticalExtent(HTMLElement expected, HTMLElement actual) {
    ...
    assertTrue("segment top differs by " + topDifference + "px", Math.abs(topDifference) <= 1.0);
    assertTrue(
        "segment height differs by " + heightDifference + "px", Math.abs(heightDifference) <= 1.0);
  }
