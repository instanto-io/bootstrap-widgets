...
public class Bootstrap3ToggleSwitchSteps {
  ...
  private void choose(ToggleSwitchBase control, boolean on) {
    Dom.click(
        wrapper(control)
            .querySelector(on ? ".bootstrap-switch-handle-off" : ".bootstrap-switch-handle-on"));
  }
  ...
  public void optionsRetained() {
    ...
    assertTrue(
        wrapper(toggle).getClassName(),
        wrapper(toggle).getClassName().contains("bootstrap-switch-large"));
