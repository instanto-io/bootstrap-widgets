...
public class Bootstrap3SelectSteps {
  ...
  public void prepare() {
    ...
    Dom.container().setAttribute("id", "select-test-host");
    host = RootPanel.get("select-test-host");
  }
  ...
  private HTMLElement button() {
    return Dom.container().querySelector("button.dropdown-toggle");
  }
  ...
  public void initial() {
    select = new Select();
    options(select);
    select.setValue("Alpha");
    attach(select);
  }

  @Then("its options and selected item correspond to the widget objects")
  public void identities() {
    assertEquals(3, select.getItemCount());
    assertSame(alpha, select.getItem(0));
    assertEquals(Arrays.asList(alpha, bravo, charlie), select.getItems());
    assertSame(alpha, select.getSelectedItem());
  }

  @When("its selection is changed silently then with notification")
  public void change() {
    select.setValue("Bravo");
    assertEquals(0, changes);
    select.setValue("Charlie", true);
    select.setValue("Charlie", true);
  }
  ...
  public void event() {
    ...
    assertSame(select, source);
    assertEquals("Charlie", select.getValue());
  }
  ...
  public void choose() {
    Dom.click(button());
    Dom.waitFor(() -> assertNotNull(choice("Bravo")));
    Dom.click(choice("Bravo"));
  }

  @Then("Bravo is selected and a value change is reported")
  public void chosen() {
    assertEquals("Bravo", select.getValue());
    assertEquals(1, changes);
    assertSame(select, source);
  }
  ...
  public void search() {
    Dom.click(button());
    Dom.type(Dom.container().querySelector(".bs-searchbox input"), "Bravo");
  }
  ...
  public void filtered() {
    Dom.waitFor(
        () -> {
          assertNotNull(choice("Bravo"));
          assertNull(choice("Alpha"));
        });
  }
  ...
  public void multi() {
    ...
    options(multiple);
    multiple.setCountSelectedTextFormat(0);
  ...
  public void several() {
    multiple.setValue(Arrays.asList("Alpha", "Bravo"));
  }

  @Then("both values are selected and the custom count is shown")
  public void severalValues() {
  ...
  public void locales() {
    multiple = new MultipleSelect();
    multiple.setLanguage(SelectLanguage.FR);
    attach(multiple);
    select = new Select();
    attach(select);
  }
  ...
  public void localised() {
    ...
    assertTrue(
        buttons.item(1).getTextContent(),
        buttons.item(1).getTextContent().contains("Nothing selected"));
  }
