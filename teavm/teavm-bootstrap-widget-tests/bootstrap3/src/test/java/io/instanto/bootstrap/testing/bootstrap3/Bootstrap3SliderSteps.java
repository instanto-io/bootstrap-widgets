...
public class Bootstrap3SliderSteps {
  ...
  private HTMLElement handle() {
    return Dom.container().querySelector(".slider-handle");
  }
  ...
  public void event() {
    ...
    assertSame(slider, source);
    assertEquals(6, slider.getValue(), 0);
  }
  ...
  public void enabled() {
    ...
    assertFalse(slider.getStyleElement().hasClassName("slider-disabled"));
  }
  ...
  public void key() {
    Dom.focus(handle());
    Dom.press(handle(), "ArrowRight");
  }

  @Then("its value increases by one")
  public void increment() {
    assertEquals(5, slider.getValue(), 0);
  }
}
