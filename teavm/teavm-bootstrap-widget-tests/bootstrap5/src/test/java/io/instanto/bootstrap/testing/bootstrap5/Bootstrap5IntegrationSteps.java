...
public class Bootstrap5IntegrationSteps {
  ...
  public void fixture(String name) {
    switch (name) {
      ...
      case "select":
        ...
        if (name.equals("remote-select"))
          select.setDataProvider((query, result) -> directoryResult = result);
        widget = select;
        break;
      case "grid":
        grid = new DataTable();
        grid.setColumns(
            Arrays.asList(new GridColumn("Name", "name"), new GridColumn("City", "city")));
        Map<String, Object> london = new LinkedHashMap<>();
        london.put("name", "Ada");
        london.put("city", "London");
        Map<String, Object> paris = new LinkedHashMap<>();
        paris.put("name", "Grace");
  ...
  private void ready() {
    Dom.waitFor(() -> assertTrue(widget.isReady()), 10000);
  }
  ...
  public void sqlSelected() {
    assertEquals(Arrays.asList("sql"), select.getValue());
  ...
  public void newTag() {
    select.setValue(Arrays.asList("new-skill"));
    reattach();
  }

  @Then("the new tag remains selected")
  public void tagPreserved() {
    assertEquals(Arrays.asList("new-skill"), select.getValue());
  }
  ...
  public void disabled() {
    ...
    assertTrue(Dom.find(".ts-control input").hasAttribute("disabled"));
  }
  ...
  public void directoryResult() {
    directoryResult.onSuccess(Arrays.asList(new SelectOption("remote", "Remote colleague")));
  }
  ...
  public void closed() {
    Dom.waitFor(() -> assertNull(HTMLDocument.current().querySelector(".pswp--open")));
  }
  ...
  public void oneIntegration() {
    assertEquals(1, host.getWidgetCount());
    assertTrue(widget.isReady());
  }
}
