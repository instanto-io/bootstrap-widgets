...
public class Bootstrap5NativeFeedbackSteps {
  ...
  public void hiddenToast() {
    assertFalse(toast.isShown());
    assertEquals(Arrays.asList("show"), events);
  }

  @When("a hide handler cancels dismissal")
  public void cancelHide() {
    toast.addHideHandler(event -> event.preventDefault());
    toast.hide();
  }

  @Then("the toast remains shown")
  public void shownToast() {
    assertTrue(toast.isShown());
    assertEquals(Arrays.asList("show", "shown", "hide"), events);
  }
  ...
  public void empty() {
    Dom.waitFor(() -> assertEquals(0, notifications.getWidgetCount()), 5000);
  }
  ...
  public void escape() {
    Dom.press(panel.getElement().unwrap(), "Escape");
  }

  @When("its panel close button is clicked")
  public void closePanel() {
    Dom.click(Dom.container().querySelector("#native-panel .btn-close"));
  }
  ...
  public void released() {
    Dom.waitFor(
        () -> {
          ...
          assertEquals(
              previousOverflow,
              HTMLDocument.current().getBody().getStyle().getPropertyValue("overflow"));
        });
  }
