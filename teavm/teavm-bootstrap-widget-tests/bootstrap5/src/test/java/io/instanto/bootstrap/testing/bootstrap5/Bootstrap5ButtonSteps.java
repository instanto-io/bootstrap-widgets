...
@CucumberSuite(value = {"features/buttons.feature", "features/widget-lifecycle.feature"})
public class Bootstrap5ButtonSteps {
  ...
  private void createFixture(String fixture, boolean mount) {
    ...
    if (fixture.startsWith("behaviour/check-box-button/")) {
      ...
      checkBoxButton.addValueChangeHandler(
          event -> {
            valueChanges++;
            lastEventSource = event.getSource();
          });
    ...
    if (fixture.equals("behaviour/radio-buttons/exclusive")) {
      radioButtons =
          new RadioButton[] {
            new RadioButton("fixture-radio", "One"), new RadioButton("fixture-radio", "Two")
          };
  ...
  private void createLifecycleFixture(String fixture, boolean mounted) {
    ...
    if (fixture.endsWith("plugin")) {
      ...
      tooltip.setContainer("#bootstrap5-widget-test-host");
    }
    if (mounted) {
  ...
  public void stableFixtureIdIsPresent() {
    ...
    expect(element(lifecycleWidget))
        .toHaveAttribute("data-testid", lifecycleWidget.getElement().getId());
  }
