...
final class StringConstantsGenerator {
    ...
    void process(Element element) {
        ...
        TypeElement base =
                env.getElementUtils()
                        .getTypeElement("com.google.gwt.i18n.client.ConstantsWithLookup");
        if (base == null
                || type.equals(base)
        ...
        try (var provider =
                env.getFiler()
                        .getResource(
                                StandardLocation.CLASS_OUTPUT, "", "META-INF/services/" + binary)
                        .openInputStream()) {
        ...
        try {
            ...
            for (Element member : env.getElementUtils().getAllMembers(type)) {
                ...
                if (method.getSimpleName().contentEquals("getString")
                        && method.getParameters().size() == 1) continue;
            ...
            java.append("public String getString(String name){ if(name!=null) switch(name){")
                    .append(cases)
                    .append(
                            "} throw new java.util.MissingResourceException(\"Unknown string"
                                + " constant\", ")
                    .append(quote(binary))
                    .append(", name); }\n}\n");
